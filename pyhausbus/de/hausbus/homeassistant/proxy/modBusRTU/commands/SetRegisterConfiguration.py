...
class SetRegisterConfiguration:
  ...
  def _fromBytes(dataIn:bytearray, offset):
    return SetRegisterConfiguration(HausBusUtils.bytesToInt(dataIn, offset), HausBusUtils.bytesToInt(dataIn, offset), ERegisterType._fromBytes(dataIn, offset), HausBusUtils.bytesToWord(dataIn, offset), ESensorType._fromBytes(dataIn, offset), ESiPrefix._fromBytes(dataIn, offset))

  def __str__(self):
    return f"SetRegisterConfiguration(idx={self.idx}, node={self.node}, registerType={self.registerType}, address={self.address}, sensorType={self.sensorType}, siPrefix={self.siPrefix})"
